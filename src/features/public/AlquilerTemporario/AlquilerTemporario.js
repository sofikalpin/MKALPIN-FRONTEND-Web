...
const MapContainer = React.memo(
  ({ propiedades, propiedadSeleccionada, onMarkerClick }) => {
      ...
      (propiedadesToDisplay) => {
        ...
        propiedadesToDisplay.forEach((propiedad) => {
          if (
            ...
            typeof propiedad.coordenadas.lat !== "number" ||
            typeof propiedad.coordenadas.lng !== "number" ||
            isNaN(propiedad.coordenadas.lat) ||
            isNaN(propiedad.coordenadas.lng)
          ) {
            console.warn(
              `Coordenadas inválidas para propiedad ID ${propiedad.id}`,
            );
            return;
          }
          ...
          const marker = L.marker(
            [propiedad.coordenadas.lat, propiedad.coordenadas.lng],
            { icon: icon, propiedadId: propiedad.id },
          ).addTo(mapRef.current);

          marker.on("click", () => onMarkerClick(propiedad));

          const displayPrice =
            propiedad.precioPorNoche || propiedad.precio || 0;
          marker.bindPopup(
            `<div class="font-semibold">${propiedad.titulo || ""}</div><div class="text-blue-600 font-medium">$${displayPrice.toLocaleString()} / Noche</div><div class="text-sm text-gray-600">${propiedad.barrio || ""}</div>`,
          );
    ...
    useEffect(() => {
      if (propiedadSeleccionada && mapRef.current) {
        if (
          propiedadSeleccionada.coordenadas &&
          typeof propiedadSeleccionada.coordenadas.lat === "number" &&
          typeof propiedadSeleccionada.coordenadas.lng === "number"
        ) {
          mapRef.current.flyTo(
            [
              propiedadSeleccionada.coordenadas.lat,
              propiedadSeleccionada.coordenadas.lng,
            ],
            15,
          );
        }
...
const AlquilerTemporario = () => {
  ...
  const fetchPropiedades = useCallback(async (apiParams = {}) => {
    ...
    try {
      const query = new URLSearchParams({
        ...
        ...(apiParams.habitacionesMin && {
          habitacionesMin: apiParams.habitacionesMin,
        }),
        ...(apiParams.tipoPropiedad && {
          tipoPropiedad: apiParams.tipoPropiedad,
        }),
      });
      ...
      if (data.status) {
        const mappedProperties = data.value.map((prop) => ({
          ...
          capacidadHuespedes:
            ...
            (prop.habitaciones ? prop.habitaciones * 2 : 1),
          servicios: prop.servicios || [],
          disponibilidad: true,
        }));
        setPropiedadesRaw(mappedProperties);
      } else {
        throw new Error(data.msg || "Error API");
      }
    } catch (err) {
      console.error("Error fetching:", err);
      setError(`No se cargaron propiedades (${err.message})`);
      setPropiedadesRaw([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    filtros.precioMin,
    filtros.precioMax,
    filtros.habitacionesFiltro,
    filtros.tipo,
  ]);

  const aplicarTodosLosFiltros = useCallback(() => {
    let currentFilteredProperties = [...propiedadesRaw];
    const totalHuespedesDeseados =
      parseInt(filtros.adultos, 10) + parseInt(filtros.niños, 10);

    if (totalHuespedesDeseados > 0) {
      currentFilteredProperties = currentFilteredProperties.filter(
        (prop) => prop.capacidadHuespedes >= totalHuespedesDeseados,
      );
    }
    if (filtros.banos) {
      currentFilteredProperties = currentFilteredProperties.filter(
        (prop) => prop.banos >= parseInt(filtros.banos, 10),
      );
    }
  ...
  const toggleFavorito = async (id, e) => {
    ...
    const updateList = (list) =>
      list.map((p) => (p.id === id ? { ...p, favorito: !p.favorito } : p));
    setPropiedadesRaw(updateList);
    setPropiedadesFiltradas(updateList);
  };

  const tiposOptions = [...new Set(propiedadesRaw.map((p) => p.tipo))]
    .filter(Boolean)
    .sort();
    ...
    <div className="flex flex-col min-h-screen bg-white-50">
      ...
      <div className="bg-gray-200 text-black py-16 mb-10 mt-1">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl font-bold mb-4">
            Alojamiento de Alquiler Temporario
          </h1>
          <p className="text-xl opacity-90 max-w-2xl mx-auto mb-8">
            Explora nuestra selección de propiedades exclusivas y encuentra tu
            hogar perfecto con nosotros.
          </p>
        </div>
      </div>
      ...
      <main className="container mx-auto p-4 flex-grow">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div
            className={`${mostrarFiltros ? "block" : "hidden"} md:block w-full md:w-1/4 transition-all duration-300`}
          >
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
              <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center">
                <Filter size={20} className="mr-2 text-blue-600" /> Filtrar
              </h2>
              <div className="space-y-5">
                <div className="space-y-3">
                  <label className="block text-sm font-semibold text-gray-700">
                    <div className="flex items-center gap-2">
                      <Calendar size={16} className="text-blue-600" /> Fechas de
                      estadía
                    </div>
                  </label>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    <div className="flex items-center gap-2">
                      <MapPin size={16} className="text-blue-600" /> Ubicación
                      (Barrio)
                    </div>
                  </label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    <div className="flex items-center gap-2">
                      <Bath size={16} className="text-blue-600" /> Baños (mín.)
                    </div>
                  </label>
                  <div className="relative">
                    ...
                    >
                      ...
                      {[...new Set(propiedadesRaw.map((p) => p.banos))]
                        .filter(Boolean)
                        .sort((a, b) => a - b)
                        .map((num) => (
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    <div className="flex items-center gap-2">
                      <Home size={16} className="text-blue-600" /> Tipo
                    </div>
                  </label>
                  <div className="relative">
                    <select
                ...
                <div className="pt-4 space-y-3">
                  <button
                    onClick={aplicarFiltrosLaterales}
          ...
          <div className="w-full md:w-3/4 space-y-6">
            <div className="bg-white p-4 rounded-xl shadow-md h-64 md:h-96 relative overflow-hidden">
              <h2 className="text-xl font-bold mb-4 text-gray-800 flex items-center">
                <MapPin size={18} className="mr-2 text-blue-600" /> Ubicación en
                el Mapa
              </h2>
              <div
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-white p-3 rounded-lg shadow-lg z-20 max-w-sm">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-blue-600 font-semibold whitespace-nowrap">
                        $
                        {(
                          propiedadSeleccionada.precioPorNoche ||
                          propiedadSeleccionada.precioOriginal
                        )?.toLocaleString() || "N/A"}{" "}
                        / Noche
                      </span>
                    </div>
                    <div className="text-xs text-gray-600 truncate">
                      {propiedadSeleccionada.barrio || "N/A"}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-800 flex items-center">
                  <Home size={20} className="mr-2 text-blue-600" /> Propiedades
                  disponibles
                </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {propiedadesFiltradas.map((prop) => (
                    <div
                      ...
                      className={`bg-white rounded-xl shadow-md overflow-hidden cursor-pointer transition-all hover:shadow-lg ${propiedadSeleccionada?.id === prop.id ? "ring-2 ring-blue-500 scale-102" : ""}`}
                    >
                      <div className="h-52 bg-gray-200 relative">
                        <img
                          src={
                            prop.imagenes && prop.imagenes.length > 0
                              ? prop.imagenes[0].rutaArchivo ||
                                prop.imagenes[0].url ||
                                prop.imagenes[0]
                        ...
                        <div className="absolute top-3 left-3 right-3 flex justify-between items-start">
                          ...
                          <button
                            onClick={(e) => toggleFavorito(prop.id, e)}
                            className={`p-2 rounded-full shadow-md transition-colors ${prop.favorito ? "bg-red-500 text-white" : "bg-white text-gray-600 hover:text-red-500"}`}
                          >
                            <Bookmark
                              size={18}
                              className={prop.favorito ? "fill-current" : ""}
                            />
                          </button>
                        </div>
                      </div>
                      <div className="p-5">
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <span className="text-lg font-bold text-blue-600 whitespace-nowrap">
                            $
                            {(
                              prop.precioPorNoche || prop.precioOriginal
                            )?.toLocaleString() || "N/A"}{" "}
                        ...
                        <p className="text-sm text-gray-600 flex items-center truncate">
                          ...
                          />
                          {prop.barrio}, {prop.localidad || prop.provincia}
                        </p>
                        ...
                        <button
                          onClick={() =>
                            navigate(`/alquilertemporario/detalle/${prop.id}`)
                          }
                          className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-all shadow-sm hover:shadow"
                        >
                          Ver detalles
                        </button>
                      </div>
