    ...
    const fetchInmueble = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/Propiedad/Obtener/${id}`,
        );
        if (response.data.status) {
          ...
          const imagenesUrls = imagenesArray
            .map((img) => {
          ...
          setInmueble({
            ...
            caracteristicas: [
              {
                icon: <FaBuilding />,
                texto: `${fetchedInmueble.superficieM2 || "N/A"} m² construidos`,
              },
              {
                icon: <FaTree />,
                texto: `${fetchedInmueble.terrenoM2 || "N/A"} m² de terreno`,
              },
              {
                icon: <FaBed />,
                texto: `${fetchedInmueble.habitaciones || "N/A"} Habitaciones`,
              },
              {
                icon: <FaBath />,
                texto: `${fetchedInmueble.banos || "N/A"} Baños`,
              },
            ],
            disponibilidad: {
              minEstadia: fetchedInmueble.estadiaMinima || 1,
              maxEstadia: 365,
              fechasOcupadas: Array.isArray(fetchedInmueble.disponibilidad)
                ? fetchedInmueble.disponibilidad
                : [],
            },
    ...
    return (
      <div className="bg-gray-50 min-h-screen flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-gray-600">
          Por favor, intenta de nuevo más tarde o verifica la URL.
        </p>
      </div>
    );
  }
  ...
  if (!inmueble) {
    return (
      <div className="bg-gray-50 min-h-screen flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-gray-600">
          Es posible que la propiedad que buscas no exista o haya sido
          eliminada.
        </p>
      </div>
    );
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/Contacto/AlquilerTemporal`,
        {
          ...formData,
          propiedadId: id,
  ...
  const Mapa = ({ lat, lng, titulo, direccion }) => {
    useEffect(() => {
      ...
      L.marker([lat, lng])
        .addTo(mapRef.current)
        .bindPopup(
          `<b>${titulo || "Propiedad"}</b><br>${direccion || "Ubicación"}`,
        )
        .openPopup();
          ...
          <div>
            ...
            <div className="mt-4">
              ...
              {activeTab === "descripcion" && (
                <div className="space-y-8">
                  ...
                  {inmueble.reglasPropiedad &&
                    inmueble.reglasPropiedad.length > 0 && (
                      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                        <h3 className="text-xl font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-100">
              ...
              {activeTab === "especificaciones" && (
                <div>
                  {inmueble.especificaciones &&
                  inmueble.especificaciones.length > 0 ? (
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    ...
                    {inmueble.servicios && inmueble.servicios.length > 0 ? (
                      ...
                      <p className="text-gray-600 text-sm">
                        Esta propiedad no tiene servicios declarados.
                      </p>
                    )}
                  </div>
                </div>
              ...
              {activeTab === "informacionEstadia" && (
                <div>
                  <div className="bg-white p-4 rounded-lg shadow-sm mb-6">
                    <h3 className="text-xl font-medium text-gray-900 mb-4 pb-2 border-b border-gray-100">
                    ...
                    <div className="space-y-3 text-gray-700">
                      <p className="flex justify-between">
                        ...
                        <span className="font-semibold text-gray-900">
                          {inmueble.disponibilidad.minEstadia} noches
                        </span>
                      </p>

                      <div className="bg-gray-50 p-3 rounded-md text-sm text-gray-600 mt-4">
                        <p>
                          Por favor, utilice el formulario de contacto para
                          consultar la disponibilidad exacta para las fechas
                          deseadas.
                        </p>

                        <p className="mt-2 font-medium text-yellow-800">
                          El valor publicado puede variar según la temporada y
            ...
            <div className="mt-8 bg-white p-6 rounded-lg shadow-sm">
              <h3 className="text-xl font-bold text-gray-900 mb-4">
                ¿Te interesa este alquiler temporario?
              </h3>
              {showSuccess && (
                <div className="mb-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded">
                  ¡Gracias por tu interés en este alquiler temporario! Hemos
                  recibido tu consulta y nos pondremos en contacto contigo a la
                  brevedad.
                </div>
              )}
