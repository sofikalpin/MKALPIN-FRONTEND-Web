...
const AlquilerDetalle = () => {
  ...
  useEffect(() => {
    const fetchPropiedadData = async () => {
      try {
        ...
        const fetchedImages = imagenesArray
          .map((img) => {
        ...
        setError(
          "Error al conectar con el servidor. Por favor, verifica la conexión y las rutas de la API.",
        );
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/Contacto/EnviarConsultaPropiedad`,
        {
          ...
          body: JSON.stringify({
            ...
            mensaje:
              formData.mensaje ||
              `Consulta sobre la propiedad: ${propiedad?.titulo || ""}`,
          }),
        },
      );
      ...
      alert(
        "Error de conexión al enviar el formulario. Por favor intenta más tarde.",
      );
    }
  };
    ...
    if (
      ...
      return (
        <p className="text-gray-500 p-4 bg-gray-100 rounded-lg">
          Ubicación geográfica no disponible o inválida.
        </p>
      );
    }
  ...
  const inmuebleDisplay = {
    ...
    precio:
      propiedad.transaccionTipo === "Alquiler"
    ...
    caracteristicas: [
      {
        icon: <FaBuilding />,
        texto: `${propiedad.superficieM2 || "N/A"} m² construidos`,
      },
      {
        icon: <FaTree />,
        texto: `${propiedad.terrenoM2 || "N/A"} m² de terreno`,
      },
      {
        icon: <FaBed />,
        texto: `${propiedad.habitaciones || "N/A"} Habitaciones`,
      },
      { icon: <FaBath />, texto: `${propiedad.banos || "N/A"} Baños` },
      {
        icon: <FaCar />,
        texto: `${propiedad.estacionamientos || "N/A"} Estacionamientos`,
      },
      {
        icon: <FaRuler />,
        texto: propiedad.cocinaEquipada
          ? "Cocina equipada"
          : "Cocina no equipada",
      },
    ...
    especificaciones: [
      {
        icon: <FaBuilding />,
        texto: `Antigüedad: ${propiedad.antiguedad || "N/A"} años`,
      },
      {
        icon: <FaSun />,
        texto: `Orientación: ${propiedad.orientacion || "N/A"}`,
      },
      {
        icon: <FaSnowflake />,
        texto: propiedad.aireAcondicionado
          ? "Aire acondicionado"
          : "No tiene A/C",
      },
      {
        icon: <FaSwimmingPool />,
        texto: propiedad.piscina ? "Piscina" : "No tiene piscina",
      },
      {
        icon: <FaLock />,
        texto: propiedad.seguridad24hs
          ? "Seguridad 24hs"
          : "No tiene seguridad 24hs",
      },
    ].filter(
      (item) => !item.texto.includes("N/A") && !item.texto.includes("No tiene"),
    ),
            ...
            <div className="grid grid-cols-5 gap-2 mt-4">
              {inmuebleDisplay.imagenes.map((imgUrl, index) => (
                <img
                  ...
                  onError={(e) => {
                    e.target.src = "/images/placeholder-50x50.png";
                    console.error("Error al cargar miniatura:", e.target.src);
                  }}
                />
              ))}
            </div>
                ...
                <div>
                  <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
                    ...
                    <p className="text-gray-700 mb-4">
                      {inmuebleDisplay.direccion || "Dirección no disponible."}
                    </p>
                    <Mapa
