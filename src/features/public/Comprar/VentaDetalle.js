...
const DetalleInmueble = () => {
  ...
  useEffect(() => {
    const fetchInmueble = async () => {
      try {
        ...
        setError(
          "Hubo un problema al conectar con el servidor: " + err.message,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/Contacto/EnviarConsultaPropiedad`,
        {
          method: "POST",
          headers: {
      ...
      alert(
        "Hubo un error al enviar tu consulta. Por favor, inténtalo nuevamente más tarde.",
      );
    }
  };
  ...
  const Mapa = ({ lat, lng, titulo, ubicacion }) => {
    useEffect(() => {
      ...
      L.marker([lat, lng], { icon: customIcon })
        .addTo(mapRef.current)
        .bindPopup(
          `<b>${titulo || "Propiedad"}</b><br>${ubicacion || "Ubicación"}`,
        )
        .openPopup();
      return () => {
        if (mapRef.current) {
    ...
    if (
      ...
      return (
        <p className="text-gray-500 p-4 bg-gray-100 rounded-lg">
          Ubicación geográfica no disponible o inválida para esta propiedad.
        </p>
      );
    }
  ...
  if (!inmueble) {
    return (
      <div className="bg-gray-50 min-h-screen flex items-center justify-center">
        <p className="text-gray-700 text-lg">
          No se encontró la propiedad con el ID {id}.
        </p>
      </div>
    );
  }
  ...
  const getCaracteristicasDisplay = (inmueble) =>
    [
      {
        icon: <FaBuilding />,
        texto: inmueble.superficieM2
          ? `${inmueble.superficieM2} m² construidos`
          : null,
      },
      {
        icon: <FaTree />,
        texto: inmueble.terrenoM2
          ? `${inmueble.terrenoM2} m² de terreno`
          : null,
      },
      {
        icon: <FaBed />,
        texto: inmueble.numeroHabitaciones
          ? `${inmueble.numeroHabitaciones} Habitaciones`
          : null,
      },
      {
        icon: <FaBath />,
        texto: inmueble.numeroBanios ? `${inmueble.numeroBanios} Baños` : null,
      },
      {
        icon: <FaCar />,
        texto: inmueble.estacionamientos
          ? `${inmueble.estacionamientos} Estacionamientos`
          : inmueble.estacionamientos === 0
            ? "Sin estacionamiento"
            : null,
      },
      {
        icon: <FaRuler />,
        texto:
          inmueble.cocinaEquipada !== undefined &&
          inmueble.cocinaEquipada !== null
            ? inmueble.cocinaEquipada
              ? "Cocina equipada"
              : "Cocina no equipada"
            : null,
      },
  ...
  const getEspecificacionesDisplay = (inmueble) =>
    [
      {
        icon: <FaBuilding />,
        texto: inmueble.antiguedad
          ? `Antigüedad: ${inmueble.antiguedad} años`
          : null,
      },
      {
        icon: <FaSun />,
        texto: inmueble.orientacion
          ? `Orientación: ${inmueble.orientacion}`
          : null,
      },
      {
        icon: <FaSnowflake />,
        texto:
          inmueble.aireAcondicionado !== undefined &&
          inmueble.aireAcondicionado !== null
            ? inmueble.aireAcondicionado
              ? "Aire acondicionado"
      ...
      },
      {
        icon: <FaSwimmingPool />,
        texto:
          inmueble.piscina !== undefined && inmueble.piscina !== null
            ? inmueble.piscina
              ? "Piscina"
              : "Sin piscina"
            : null,
      },
      {
        icon: <FaLock />,
        texto:
          inmueble.seguridad24hs !== undefined &&
          inmueble.seguridad24hs !== null
            ? inmueble.seguridad24hs
              ? "Seguridad 24hs"
              : "Sin seguridad"
            : null,
      },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-10 mt-10">
          <h1 className="text-3xl font-bold text-gray-900">
            {inmueble.titulo || "Título no disponible"}
          </h1>
          <p className="mt-2 text-gray-600">
            {inmueble.ubicacion || "Dirección no disponible"}
          </p>
          <p className="mt-4 text-2xl font-semibold text-gray-900">
            $
            {inmueble.precio?.toLocaleString("es-AR") || "Precio no disponible"}
          </p>
        </div>
              ...
              {activeTab === "caracteristicas" && (
                <div className="grid grid-cols-2 gap-4">
                  {getCaracteristicasDisplay(inmueble).map((item, index) => (
                    ...
                    >
                      <span className="text-gray-700 text-xl mr-3">
                        {item.icon}
                      </span>
              ...
              {activeTab === "especificaciones" && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {getEspecificacionesDisplay(inmueble).length === 0 && (
                    <p className="text-gray-500 col-span-2">
                      No hay detalles o especificaciones disponibles.
                    </p>
                  )}
                </div>
              )}
                ...
                <div>
                  <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
                    ...
                    <Mapa
                      ...
                      ubicacion={
                        inmueble.ubicacion || buildDisplayAddress(inmueble)
                      }
                    />
                  </div>
                </div>
            ...
            <div className="mt-8 bg-white p-4 rounded-lg shadow-sm">
              ...
              {showSuccess && (
                <div className="mb-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded">
                  ¡Gracias por tu interés en esta propiedad! Hemos recibido tu
                  consulta y nos pondremos en contacto contigo a la brevedad.
                </div>
              )}
