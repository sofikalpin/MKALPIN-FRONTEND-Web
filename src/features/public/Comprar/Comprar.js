...
const Comprar = () => {
  ...
  const fetchPropiedades = useCallback(
    async (criterios = {}) => {
      ...
      queryString.append(
        "transaccionTipo",
        criterios.transaccionTipo || filtros.transaccionTipo,
      );

      if (criterios.barrio) queryString.append("barrio", criterios.barrio);
      if (criterios.precioMin)
        queryString.append("precioMin", criterios.precioMin);
      if (criterios.precioMax)
        queryString.append("precioMax", criterios.precioMax);
      if (criterios.habitaciones)
        queryString.append("habitacionesMin", criterios.habitaciones);
    ...
    (propsToDisplay) => {
      ...
      if (propsToDisplay.length > 0) {
        const bounds = L.latLngBounds(
          propsToDisplay.map((p) => [p.coordenadas.lat, p.coordenadas.lng]),
        );
  ...
  const barrios = [...new Set(propiedades.map((p) => p.barrio))].filter(
    Boolean,
  );
  const tipos = [...new Set(propiedades.map((p) => p.tipoPropiedad))].filter(
    Boolean,
  );
  const habitacionesOptions = [
    ...
    .filter(Boolean)
    .sort((a, b) => a - b);
  const banosOptions = [...new Set(propiedades.map((p) => p.banos))]
    .filter(Boolean)
    .sort((a, b) => a - b);
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-white p-3 rounded-lg shadow-lg z-20">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-blue-600 font-semibold">
                      ${propiedadSeleccionada.precio?.toLocaleString() || "N/A"}
                    </span>
                  </div>
              ...
              {loading ? (
                ...
                <div className="bg-white p-8 rounded-xl shadow-md text-center">
                  ...
                  <p className="text-gray-600 mt-2">
                    Intenta modificar los filtros de búsqueda o limpiar los
                    filtros.
                  </p>
                </div>
              ) : (
                  ...
                  {propiedadesFiltradas.map((propiedad) => (
                    ...
                    >
                      ...
                      <div className="p-5">
                        ...
                        <button
                          onClick={() =>
                            navigate(`/venta/detalle/${propiedad.idPropiedad}`)
                          }
