...
const HomeSearch = ({
  ...
  const [localAvailablePropertyTypes, setLocalAvailablePropertyTypes] =
    useState([]);
  const [locationSuggestions, setLocationSuggestions] = useState([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [availableLocations, setAvailableLocations] = useState([]);
  const [locationsByTab, setLocationsByTab] = useState({
    venta: [],
    alquiler: [],
    alquilerTemp: [],
  });
  const [propertyTypesByTab, setPropertyTypesByTab] = useState({
    venta: [],
    alquiler: [],
    alquilerTemp: [],
  });
      ...
      try {
        ...
        if (data.status && Array.isArray(data.value)) {
          ...
          data.value.forEach((prop) => {
            ...
            const esTemporario =
              !!prop.esAlquilerTemporario || transaccion.includes("temporario");
  ...
  useEffect(() => {
    ...
    const currentLocations = tabLocations.length
      ? tabLocations
      : fallbackLocations;
    ...
    if (propertyType && !currentTypes.includes(propertyType))
      setPropertyType("");
  ...
  const handleSearchWithValidation = () => {
    ...
    const matchedLocation =
      availableLocations.find(
        (location) => location.toLowerCase() === normalizedValue,
      ) ||
      ...
      <div className="relative">
        <input
          ...
          placeholder={
            id.includes("temp")
              ? "Ingresa un destino"
              : "Ingresa una ubicación o barrio"
          }
          value={inputValue}
          ...
          onFocus={() => {
            ...
            if (
              inputValue.length > 0 &&
              (locationSuggestions.length > 0 || isLoading)
            ) {
              setShowSuggestions(true);
            }
          }}
  ...
  const GuestsSelector = () => (
    <div className="relative">
      ...
      >
        <span>
          {guestsInfo.adults + guestsInfo.children} huéspedes,{" "}
          {guestsInfo.rooms} habitaciones
        </span>
        ...
        <div className="absolute z-10 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-200 p-4">
          <div className="space-y-4">
            <GuestCounter
              ...
              onDecrease={() =>
                handleGuestChange("adults", guestsInfo.adults - 1)
              }
              onIncrease={() =>
                handleGuestChange("adults", guestsInfo.adults + 1)
              }
            ...
            <GuestCounter
              ...
              onDecrease={() =>
                handleGuestChange("children", guestsInfo.children - 1)
              }
              onIncrease={() =>
                handleGuestChange("children", guestsInfo.children + 1)
              }
            ...
            <GuestCounter
              ...
              onDecrease={() =>
                handleGuestChange("rooms", guestsInfo.rooms - 1)
              }
              onIncrease={() =>
                handleGuestChange("rooms", guestsInfo.rooms + 1)
              }
          ...
          {activeTab === "alquilerTemp" ? (
            ...
            <>
              <div className="md:col-span-2">
                <LocationInput
                  id="main-destination"
                  label="¿Qué zona te encuentras interesado?"
                  required
                  inputRef={searchInputRef}
                />
              </div>
