...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setMessage(
        "Token inválido. Por favor, solicita restablecer la contraseña de nuevo.",
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/Usuario/reestablecer-contrasena`,
        {
          method: "POST",
          headers: {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center py-8 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="w-full max-w-md space-y-4 relative z-10">
        <div className="bg-white shadow-lg rounded-lg border border-gray-200 p-8 sm:p-10">
          ...
          {message && (
            <div
              className={`mt-6 p-4 rounded-md text-center text-sm font-medium transition-colors ${
