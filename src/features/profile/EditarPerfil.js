...
export const EditarPerfil = ({ onUpdate }) => {
    ...
    switch (name) {
      ...
      case "apellido":
        if (!inputValue) {
          ...
          error =
            "Debe contener solo letras, espacios, tildes, apóstrofes y guiones. Entre 2 y 50 caracteres.";
        }
        break;
      case "telefono":
        if (inputValue) {
  ...
  const cargarPerfil = async () => {
    try {
      ...
      if (error.response) {
        alert(
          `Error ${error.response.status}: ${error.response.data.message || "No se pudieron cargar los datos del perfil."}`,
        );
      } else {
        alert(error.message);
      }
  ...
  const handleActualizar = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        alert(
          `Error ${error.response.status}: ${error.response.data.message || "No se pudo actualizar el perfil."}`,
        );
      }
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-xl bg-white shadow-2xl rounded-3xl overflow-hidden">
        <div className="bg-white border-b border-slate-200 p-6 text-center relative flex flex-col items-center">
          ...
          />
          <h3 className="text-2xl font-semibold text-slate-800 tracking-wide">
            Editar Perfil
          </h3>
        </div>
          ...
          <form onSubmit={handleActualizar} className="p-10 space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  name="nombre"
                  type="text"
                  className={`w-full px-4 py-3 border rounded-lg focus:ring-2 ${errors.nombre ? "border-red-500 focus:ring-red-400" : "border-gray-300 focus:ring-gray-400"}`}
                />
                {errors.nombre && (
              ...
              <div>
                ...
                <input
                  ...
                  name="apellido"
                  type="text"
                  className={`w-full px-4 py-3 border rounded-lg focus:ring-2 ${errors.apellido ? "border-red-500 focus:ring-red-400" : "border-gray-300 focus:ring-gray-400"}`}
                />
                {errors.apellido && (
            ...
            <div>
              ...
              <input
                ...
                disabled
                type="email"
                className={`w-full px-4 py-3 border rounded-lg bg-gray-100 cursor-not-allowed ${errors.email ? "border-red-500" : "border-gray-300"}`}
              />
              {errors.email && (
            ...
            <div>
              ...
              <input
                ...
                name="telefono"
                type="text"
                className={`w-full px-4 py-3 border rounded-lg focus:ring-2 ${errors.telefono ? "border-red-500 focus:ring-red-400" : "border-gray-300 focus:ring-gray-400"}`}
              />
              {errors.telefono && (
            ...
            <div className="flex space-x-4 pt-4">
              <button
                type="submit"
                className="w-full py-3 bg-gray-700 text-white rounded-lg hover:bg-gray-800"
              >
                Actualizar Perfil
              </button>
              <button
