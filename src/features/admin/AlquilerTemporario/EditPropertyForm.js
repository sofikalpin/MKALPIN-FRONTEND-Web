...
const BASE_SERVICES = loadFromLocalStorage("globalServices", [
  "WiFi",
  "Limpieza general",
  "Estacionamiento",
  "Kit de Bienvenida",
...
const availableReglas = loadFromLocalStorage("globalRules", [
  "No fumar",
  "No mascotas",
...
const EditPropertyForm = ({ property, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (property) {
      ...
      const initialEstadiaMinima =
        rawEstadiaMinima === "" ||
        rawEstadiaMinima === null ||
        rawEstadiaMinima === undefined
          ? ""
      ...
      setFormData({
        ...
        precioPorSemana:
          property.pricePerWeek || property.precioPorSemana || "",
        ...
        capacidadPersonas:
          property.capacity || property.capacidadPersonas || "",
        estado: property.status || property.estado || "Disponible",
        servicios: Array.isArray(property.services)
          ? property.services
          : Array.isArray(property.servicios)
            ? property.servicios
            : [],
        reglasPropiedad: Array.isArray(property.rules)
          ? property.rules
          : Array.isArray(property.reglasPropiedad)
            ? property.reglasPropiedad
            : [],
        horarioCheckIn:
          property.checkInTime || property.horarioCheckIn || "15:00",
        horarioCheckOut:
          property.checkOutTime || property.horarioCheckOut || "11:00",
  ...
  const handleSubmit = (e) => {
    ...
    const finalEstadiaMinima =
      isNaN(parsedEstadiaMinima) || parsedEstadiaMinima < 1
        ? 1
        : parsedEstadiaMinima;
    ...
    const finalProperty = {
      ...
      precio:
        formData.precioPorNoche === ""
          ? null
          : parseFloat(formData.precioPorNoche),
      precioPorNoche:
        formData.precioPorNoche === ""
          ? null
          : parseFloat(formData.precioPorNoche),
      precioPorSemana:
        formData.precioPorSemana === ""
          ? null
          : parseFloat(formData.precioPorSemana),
      precioPorMes:
        formData.precioPorMes === "" ? null : parseFloat(formData.precioPorMes),
      capacidadPersonas:
        formData.capacidadPersonas === ""
          ? 0
          : parseInt(formData.capacidadPersonas),
      habitaciones:
        formData.habitaciones === "" ? 0 : parseInt(formData.habitaciones),
      banos: formData.banos === "" ? 0 : parseInt(formData.banos),
      superficieM2:
        formData.superficieM2 === "" ? 0 : parseInt(formData.superficieM2),
  ...
  const selectedCurrencySymbol =
    AVAILABLE_CURRENCIES.find((c) => c.code === formData.currency)?.symbol ||
    "$";
                  ...
                  <div key={index} className="relative group">
                    ...
                    >
                      <img
                        ...
                        onError={(e) => {
                          e.target.src =
                            "https://via.placeholder.com/100?text=Error+loading+image";
                        }}
                      />
