...
const ReservationCalendar = ({ property, onReserve }) => {
  ...
  const { days, isDateReserved, getReservationForDate } = useCalendar(
    currentDate,
  ...
  const handleDateClick = (date) => {
    ...
    if (isSameDay(date, selectedDates.start)) {
      ...
      for (
        let d = addDays(selectedDates.start, 1);
        d <= date;
        d = addDays(d, 1)
      ) {
        if (isDateReserved(d)) {
  ...
  const getDayClassName = (day) => {
    const classes = [
      "p-1 text-center rounded-full cursor-pointer transition-all text-sm w-8 h-8 flex items-center justify-center",
    ];
    ...
    if (format(day, "M") !== format(currentDate, "M")) {
      ...
      classes.push(
        "bg-red-500 text-white font-bold opacity-75 cursor-not-allowed",
      );
    ...
    <div className="bg-white p-4 rounded-lg shadow-xl max-w-md mx-auto">
      ...
      <div className="grid grid-cols-7 gap-2">
        {["lun", "mar", "mié", "jue", "vie", "sáb", "dom"].map((d) => (
          <div
            key={d}
            className="text-center font-semibold text-gray-500 text-xs capitalize"
          >
            {d}
          </div>
        ))}
      ...
      {selectedDates.start &&
        selectedDates.end &&
        !getReservationForDate(selectedDates.start) && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
            <h4 className="font-bold mb-2 text-center text-gray-700">
              Confirmar Reserva
            </h4>
