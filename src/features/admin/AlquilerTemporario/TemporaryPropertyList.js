...
const TemporaryPropertyList = ({
    ...
    const loadPropertyAvailability = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/Propiedad/Disponibilidad/${propertyId}`,
          {
            method: "GET",
            headers: {
        ...
        if (data.status && data.value) {
          const availability =
            data.value.availability || data.value.disponibilidad || [];

          const normalizedAvailability = availability.map((range) => ({
            ...range,
            status: range.status
              ? String(range.status).toLowerCase()
              : "disponible",
          }));
          ...
          throw new Error(
            data.message || "La respuesta del servidor no fue válida",
          );
        }
      } catch (error) {
  ...
  const filteredProperties = propertiesArray.filter((property) => {
    ...
    return (
      ...
      (property.neighborhood &&
        property.neighborhood.toLowerCase().includes(searchTermLower))
    );
  });
  ...
  const handleDeleteRange = async (rangeId) => {
    ...
    let confirmMessage =
      `¿Está seguro de que desea eliminar este rango de fechas?\n\n` +
    ...
    try {
      ...
      const deleteResponse = await fetch(
        `${API_BASE_URL}/Propiedad/Disponibilidad/${selectedProperty.id || selectedProperty._id}/${rangeId}`,
        {
          method: "DELETE",
          headers: {
            Accept: "application/json",
      ...
      if (!deleteResponse.ok) {
        ...
        throw new Error(
          errorData.message || "Error al eliminar el rango de fechas",
        );
      }

      if (!isAvailable) {
        const markAvailableResponse = await fetch(
          `${API_BASE_URL}/Propiedad/Disponibilidad/${selectedProperty.id || selectedProperty._id}`,
          {
            method: "PUT",
            headers: {
              Accept: "application/json",
        ...
        if (!markAvailableResponse.ok) {
          const errorData = await markAvailableResponse
            .json()
            .catch(() => ({}));
          throw new Error(
            errorData.message || "Error al marcar las fechas como disponibles",
          );
        }
      }
    ...
    return availabilityData.availability.some((range) => {
      ...
      try {
        ...
        const isOccupiedStatus =
          statusLower === "ocupado_temp" || statusLower === "ocupado";
    ...
    return availabilityData.availability.some((range) => {
      ...
      try {
        ...
        const isReservedStatus =
          statusLower === "reservado_temp" || statusLower === "reservado";
  ...
  const sendAvailabilityUpdate = async (
    status,
    startDate,
    endDate,
    clientName,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Error al actualizar la disponibilidad",
        );
      }
      ...
      console.error(
        `Error en sendAvailabilityUpdate (status: ${status}):`,
        error,
      );
      toast.error(error.message);
      return false;
  ...
  const handleReserveProperty = async () => {
    ...
    if (!clientName || !reservationGuests) {
      toast.error(
        "Por favor complete el Nombre del Cliente y el Número de Huéspedes.",
      );
      return;
    }

    const maxGuests = parseInt(
      selectedProperty.capacity || selectedProperty.capacidadPersonas || 1,
    );
    if (parseInt(reservationGuests) > maxGuests) {
      toast.error(
        `La cantidad de huéspedes supera la capacidad máxima de la propiedad (${maxGuests}).`,
      );
      return;
    }
    ...
    if (isRangeBooked(selectedDates.startDate, selectedDates.endDate)) {
      toast.error(
        "Error: El rango seleccionado se superpone con fechas que ya están reservadas u ocupadas.",
      );
      return;
    }
    if (
      !isRangeFullyAvailable(selectedDates.startDate, selectedDates.endDate)
    ) {
      toast.error(
        'Error: Solo puede reservar u ocupar fechas que estén explícitamente marcadas como "Disponibles".',
      );
      return;
    }
  ...
  const handleOccupyProperty = async () => {
    ...
    if (isRangeOccupied(selectedDates.startDate, selectedDates.endDate)) {
      toast.error(
        "Error: El rango seleccionado se superpone con fechas que ya están ocupadas.",
      );
      return;
    }
    if (
      !isRangeBookableOrOccupiable(
        selectedDates.startDate,
        selectedDates.endDate,
      )
    ) {
      toast.error(
        'Error: Solo puede ocupar fechas que estén explícitamente marcadas como "Disponibles" o "Reservadas".',
      );
      return;
    }
    const success = await sendAvailabilityUpdate(
  ...
  const handleDeleteDate = async (date) => {
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/Propiedad/Disponibilidad/${selectedProperty.id || selectedProperty._id}/date/${formattedDate}`,
          {
            method: "DELETE",
            headers: {
              Accept: "application/json",
  ...
  const renderDayContents = (day, date) => {
    const baseClasses =
      "rounded-full w-9 h-9 flex items-center justify-center font-medium";
    ...
    if (isOccupied) {
      dayContent = (
        <div
          className={`bg-red-200 text-red-900 border-2 border-red-400 ${baseClasses} cursor-pointer hover:bg-red-300`}
        >
          {day}
        </div>
      );
      ...
      dayContent = (
        <div
          className={`bg-yellow-200 text-yellow-900 border-2 border-yellow-400 ${baseClasses} cursor-pointer hover:bg-yellow-300`}
        >
          {day}
        </div>
      );
      ...
      dayContent = (
        <div
          className={`bg-green-200 text-green-900 border-2 border-green-400 ${baseClasses} cursor-pointer hover:bg-green-300`}
        >
          {day}
        </div>
      );
  ...
  const handleDateChange = (dates) => {
    ...
    if (
      start &&
      selectedDates.startDate &&
      start.getTime() === selectedDates.startDate.getTime() &&
      !end
    ) {
      setSelectedDates({
        startDate: null,
    ...
    if (
      ...
      selectedDates.endDate &&
      end.getTime() === selectedDates.endDate.getTime()
    ) {
      ...
      {viewMode === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProperties.map((property, index) => {
            ...
            const displayPrice =
              property.price ||
              property.precioPorNoche ||
              property.pricePerWeek ||
              property.pricePerMonth ||
              0;
            const priceDescription =
              property.price || property.precioPorNoche
                ? "por noche"
                : property.pricePerWeek
                  ? "por semana"
                  : property.pricePerMonth
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl shadow-md overflow-hidden transform hover:scale-[1.02] transition-all duration-300 hover:shadow-lg ${
                  ...
                  currentStatus === "no disponible"
                    ? "opacity-80 border-l-4 border-red-500"
                    : ""
                } ${
                ...
                <div className="p-6">
                  <div className="flex flex-col mb-4">
                    ...
                    <p className="text-sm text-gray-500">
                      {(property.neighborhood || property.barrio) &&
                        `${property.neighborhood || property.barrio}, `}
                      {(property.locality || property.localidad) &&
                        `${property.locality || property.localidad}, `}
                        ...
                        <div className="space-y-2">
                          {property.availability.map((range, index) => {
                            ...
                            if (
                              ...
                              statusLower === "reservado_temp" ||
                              statusLower === "reservado"
                            ) {
                              statusBadge = (
          ...
          <div className="bg-white rounded-lg w-full max-w-6xl p-6 overflow-y-auto max-h-[95vh]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              <div className="lg:col-span-2 space-y-4">
                ...
                <h3 className="text-2xl font-bold text-gray-800 border-b pb-2 mt-6">
                  Información General
                </h3>
                <div className="grid grid-cols-2 gap-y-2 text-gray-600 text-base">
                  <p>
                    <span className="font-semibold">Dirección:</span>{" "}
                    {selectedProperty.address || selectedProperty.direccion}
                  </p>
                  <p>
                    <span className="font-semibold">Tipo:</span>{" "}
                    {selectedProperty.type || selectedProperty.tipoPropiedad}
                  </p>
                  <p>
                    <span className="font-semibold">Localidad:</span>{" "}
                    {selectedProperty.locality || selectedProperty.localidad}
                  </p>
                  <p>
                    <span className="font-semibold">Barrio:</span>{" "}
                    {selectedProperty.neighborhood || selectedProperty.barrio}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-3">
                  {(
                    selectedProperty.services || selectedProperty.servicios
                  )?.map((service, index) => (
                    <div
                      key={index}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                  {(
                    selectedProperty.rules || selectedProperty.reglasPropiedad
                  )?.map((rule, index) => (
                    <div
                      key={index}
                      className="flex items-center text-base text-gray-700"
                ...
                <div className="p-4 border rounded-lg bg-white shadow">
                  <h3 className="text-xl font-bold mb-3 flex items-center">
                    <FaCalendarAlt className="mr-2 text-blue-500" /> Calendario
                    de Disponibilidad
                  </h3>
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="text-sm text-blue-900">
                          <p className="font-semibold mb-1">
                            {" "}
                            Tip: Funcionalidad de Doble Click
                          </p>
                          <ul className="list-disc list-inside space-y-1 text-blue-800">
                            <li>
                              <strong>Fechas Disponibles (verde):</strong> Doble
                              click para marcar como "No Disponible"
                            </li>
                            <li>
                              <strong>
                                Fechas Reservadas (amarillo) u Ocupadas (rojo):
                              </strong>{" "}
                              Doble click para marcar como "Disponible"
                            </li>
                          </ul>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            setReservationDeposit(e.target.value)
                          }
                ...
                <div className="p-4 border rounded-lg bg-white shadow">
                  <h3 className="text-xl font-bold mb-3 flex items-center">
                    <FaCalendarCheck className="mr-2 text-blue-500" /> Reservas
                    y Ocupaciones
                  </h3>
                  ...
                  availabilityData.availability.filter(
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                <p className="font-semibold text-lg">
                                  {format(parseISO(range.startDate), "PPP", {
                                    locale: es,
                                  })}{" "}
                                  -{" "}
                                  {format(parseISO(range.endDate), "PPP", {
                                    locale: es,
                                  })}
                                </p>
                              ...
                              <button
                                onClick={() =>
                                  handleDeleteRange(range.id || range._id)
                                }
                    ...
                    <p className="text-gray-500 text-base">
                      No hay reservas u ocupaciones registradas.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="lg:col-span-1 space-y-4">
                <div className="p-4 rounded-lg shadow-inner bg-gray-50">
                  ...
                  <div className="space-y-2 text-base">
                    <p>
                      ...
                      {selectedProperty.checkInTime || "14:00"}
                    </p>
                    <p>
                      <span className="font-semibold">Salida:</span>{" "}
                      {selectedProperty.checkOutTime || "10:00"}
                    </p>
                    <p>
                      <span className="font-semibold">Estadía mínima:</span>{" "}
                      {selectedProperty.estadiaMinima || 1}
                    </p>
                    {selectedProperty.availableFrom && (
                      <p>
                        <span className="font-semibold">Disponible desde:</span>{" "}
                        {new Date(
                          selectedProperty.availableFrom,
                        ).toLocaleDateString()}
                      </p>
                    ...
                    {selectedProperty.availableTo && (
                      <p>
                        <span className="font-semibold">Disponible hasta:</span>{" "}
                        {new Date(
                          selectedProperty.availableTo,
                        ).toLocaleDateString()}
                      </p>
                  ...
                  <div className="mt-4 pt-3 border-t">
                    <h3 className="text-xl font-bold mb-3 flex items-center">
                      <FaCalendarCheck className="mr-2 text-blue-500" /> Estado
                      Actual
                    </h3>
                    <div className="text-center">
                      {mapStatus(selectedProperty.estado) === "disponible" && (
                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-lg font-medium bg-green-100 text-green-800">
                          Disponible
                        </span>
                      )}
                      {mapStatus(selectedProperty.estado) ===
                        "reservado_temp" && (
                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-lg font-medium bg-yellow-100 text-yellow-800">
                          Reservado
                        </span>
                      )}
                      {mapStatus(selectedProperty.estado) ===
                        "ocupado_temp" && (
                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-lg font-medium bg-red-100 text-red-800">
                          Ocupado
                        </span>
                      )}
                      {mapStatus(selectedProperty.estado) ===
                        "no disponible" && (
                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-lg font-medium bg-gray-100 text-gray-800">
                          No disponible
                        </span>
                      )}
                    </div>
                ...
                <div className="p-4 border rounded-lg bg-white shadow">
                  <h3 className="text-xl font-bold mb-3 flex items-center">
                    <FaMoneyBill className="mr-2 text-blue-500" /> Precios y
                    Moneda
                  </h3>
                  <div className="space-y-2 text-gray-700 text-base">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>
                        ...
                        {(
                          ...
                          selectedProperty.precioPorNoche
                        )?.toLocaleString() || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span>
                        $
                        {(
                          selectedProperty.pricePerWeek ||
                          selectedProperty.precioPorSemana
                        )?.toLocaleString() || "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span>
                        $
                        {(
                          selectedProperty.pricePerMonth ||
                          selectedProperty.precioPorMes
                        )?.toLocaleString() || "N/A"}
                      </span>
                    </div>
                    {selectedProperty.cleaningFee > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span>
                          ${selectedProperty.cleaningFee?.toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedProperty.securityDeposit > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span>
                          ${selectedProperty.securityDeposit?.toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
