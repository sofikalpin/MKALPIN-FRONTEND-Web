    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 transition-opacity duration-300">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-sm p-6 transform transition-all duration-300 scale-100">
        ...
        <p className="text-gray-700 mb-6">
          Estás a punto de cancelar el registro.{" "}
          <strong className="font-extrabold text-black-600">
            Perderás todos los datos ingresados
          </strong>{" "}
          en el formulario. ¿Estás seguro?
        </p>
...
const AddPropertyForm = ({ onAddProperty, onCancel, isSubmitting = false }) => {
    ...
    try {
      ...
      const finalProperty = {
        ...
        habitaciones: property.habitaciones
          ? parseInt(property.habitaciones)
          : 0,
        banos: property.banos ? parseInt(property.banos) : 0,
        superficieM2: property.superficieM2
          ? parseFloat(property.superficieM2)
          : 0,
        capacidadPersonas: property.capacidadPersonas
          ? parseInt(property.capacidadPersonas)
          : 1,

        precioPorNoche: property.precioPorNoche
          ? parseFloat(property.precioPorNoche)
          : null,
        precioPorSemana: property.precioPorSemana
          ? parseFloat(property.precioPorSemana)
          : null,
        precioPorMes: property.precioPorMes
          ? parseFloat(property.precioPorMes)
          : null,
        estadiaMinima: property.estadiaMinima
          ? parseInt(property.estadiaMinima)
          : 1,

        estado: property.estado || "Disponible",
        activo: property.activo,
        esAlquilerTemporario:
          property.transaccionTipo === "Alquiler Temporario",
        ...
        reglasPropiedad: Array.isArray(property.reglasPropiedad)
          ? property.reglasPropiedad
          : [],
      ...
      alert(
        "Ocurrió un error al procesar el formulario. Por favor, intente nuevamente.",
      );
    }
  };
  ...
  const selectedCurrencySymbol =
    AVAILABLE_CURRENCIES.find((c) => c.code === property.currency)?.symbol ||
    "$";

  const currentCustomServices = property.servicios.filter(
    (s) => !BASE_SERVICES.includes(s) && !customServices.includes(s),
  );

  const currentCustomRules = property.reglasPropiedad.filter(
    (r) => !availableReglas.includes(r) && !customRules.includes(r),
  );
    ...
    <div className="bg-white rounded-xl shadow-lg p-8">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 p-4 border rounded-lg bg-gray-50">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Hermoso Apartamento..."
              required
              minLength={5}
            />
          ...
          <div>
            ...
            <select
              ...
              id="transaccionTipo"
              value={property.transaccionTipo}
              onChange={handleInputChange}
              className="shadow border rounded-lg w-full py-3 px-4 text-gray-700"
              required
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={handleInputChange}
              className="shadow border rounded-lg w-full py-3 px-4 text-gray-700"
              placeholder="Ej: Av. 10 Norte 245"
              required
              minLength={5}
            />
            <p className="text-xs text-gray-500 mt-1">Mínimo 5 caracteres</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 p-4 border rounded-lg">
          <h3 className="col-span-full text-lg font-semibold text-gray-800 flex items-center space-x-2">
            <FaRulerCombined />
        ...
        <div className="mb-6 p-4 border border-green-400 bg-green-50 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs text-gray-600 block mb-1">
                Capacidad Máxima en este Rango *
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="Ej: 4"
                  value={availability.availableGuests}
                  onChange={handleAvailabilityChange}
                  className="shadow border rounded-lg w-full py-2 px-3 text-gray-700 pr-8"
                  min="1"
                  required
                />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            ...
            <div>
              ...
              <input
                ...
                id="horarioCheckOut"
                value={property.horarioCheckOut}
                onChange={handleInputChange}
                className="shadow border rounded-lg w-full py-2 px-3 text-gray-700"
              />
            </div>
        ...
        <div className="mb-6 p-4 border border-gray-400 rounded-lg bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input
              type="number"
              name="precioPorNoche"
              placeholder={`Noche (${selectedCurrencySymbol})`}
              value={property.precioPorNoche}
              onChange={handleInputChange}
              className="shadow border rounded-lg w-full py-3 px-4 text-gray-700"
              min="0"
            />
            <input
              ...
              placeholder={`Semana (${selectedCurrencySymbol})`}
              value={property.precioPorSemana}
              onChange={handleInputChange}
              className="shadow border rounded-lg w-full py-3 px-4 text-gray-700"
              min="0"
            />
            <input
              type="number"
              name="precioPorMes"
              placeholder={`Mes (${selectedCurrencySymbol})`}
              value={property.precioPorMes}
              onChange={handleInputChange}
              className="shadow border rounded-lg w-full py-3 px-4 text-gray-700"
              min="0"
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="p-4 border rounded-lg">
            <label className="block text-gray-700 text-sm font-bold mb-3 flex items-center space-x-2">
              <FaTools />
              <span>Servicios y Especificaciones</span>
            </label>

            <div className="p-3 border rounded-lg bg-gray-50 max-h-48 overflow-y-auto">
              <p className="font-semibold text-xs mb-2 text-gray-700 border-b pb-1">
            ...
            >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyPress={(e) =>
                    e.key === "Enter" && handleAddNewService(e)
                  }
                />
                <button
                  type="button"
            ...
            {currentCustomServices.length > 0 && (
              <div className="mt-3 p-3 border-t border-gray-200">
                <p className="font-semibold text-xs mb-2 text-gray-700">
                  Servicios Adicionales de esta propiedad:
                </p>
                <div className="flex flex-wrap gap-2">
                  {currentCustomServices.map((service) => (
                    <div
                      key={service}
                      className="text-xs bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full flex items-center shadow-sm"
                    >
                      {service}
                      <button
                        type="button"
          ...
          <div className="p-4 border rounded-lg">
            <label className="block text-gray-700 text-sm font-bold mb-3 flex items-center space-x-2">
              <FaExclamationTriangle />
              <span>Reglas de la Propiedad</span>
            </label>
            ...
            {currentCustomRules.length > 0 && (
              <div className="mt-3 p-3 border-t border-gray-200">
                <p className="font-semibold text-xs mb-2 text-gray-700">
                  Reglas Adicionales de esta propiedad:
                </p>
                <div className="flex flex-wrap gap-2">
                  {currentCustomRules.map((rule) => (
                    <div
                      key={rule}
                      className="text-xs bg-amber-100 text-amber-700 px-3 py-1 rounded-full flex items-center shadow-sm"
                    >
                      {rule}
                      <button
                        type="button"
