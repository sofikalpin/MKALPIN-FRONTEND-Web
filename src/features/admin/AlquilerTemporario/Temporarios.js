...
const Temporarios = () => {
    ...
    const fetchTemporaryRentals = async () => {
      try {
        ...
        if (response.status && response.value) {
            ...
            response.value.map(async (prop) => {
              try {
                const availabilityResponse = await fetch(
                  `/API/Propiedad/Disponibilidad/${prop._id || prop.id}`,
                  {
                    headers: {
                ...
                const mappedProperty = {
                  ...
                  description:
                    prop.description || prop.descripcion || "Sin descripción",
                  address: prop.address || prop.direccion || "Sin dirección",
                  neighborhood:
                    prop.neighborhood || prop.barrio || "Sin barrio",
                  ...
                  rules: Array.isArray(prop.reglasPropiedad)
                    ? prop.reglasPropiedad
                    : [],
                  ...
                  estadiaMinima:
                    prop.estadiaMinima === null ||
                    prop.estadiaMinima === undefined ||
                    prop.estadiaMinima === 0
                      ? 1
                      : prop.estadiaMinima,
                  ...
                  ...(prop.especificaciones && {
                    specifications: prop.especificaciones,
                  }),
                  ...(prop.latitud && {
                    latitude: parseFloat(prop.latitud) || 0,
                  }),
                  ...(prop.longitud && {
                    longitude: parseFloat(prop.longitud) || 0,
                  }),
          ...
          setError(
            "No se pudieron cargar las propiedades de alquiler temporario",
          );
    ...
    try {
      ...
      const propertyToSave = {
        ...
        habitaciones: propertyData.habitaciones
          ? parseInt(propertyData.habitaciones)
          : 0,
        banos: propertyData.banos ? parseInt(propertyData.banos) : 0,
        superficieM2: propertyData.superficieM2
          ? parseFloat(propertyData.superficieM2)
          : 0,
        capacidadPersonas: propertyData.capacidadPersonas
          ? parseInt(propertyData.capacidadPersonas)
          : 1,

        precio: propertyData.precioPorNoche
          ? parseFloat(propertyData.precioPorNoche)
          : 0,
        precioPorNoche: propertyData.precioPorNoche
          ? parseFloat(propertyData.precioPorNoche)
          : 0,
        precioPorSemana: propertyData.precioPorSemana
          ? parseFloat(propertyData.precioPorSemana)
          : 0,
        precioPorMes: propertyData.precioPorMes
          ? parseFloat(propertyData.precioPorMes)
          : 0,
        ...
        servicios: Array.isArray(propertyData.servicios)
          ? propertyData.servicios
          : [],
        reglasPropiedad: Array.isArray(propertyData.reglasPropiedad)
          ? propertyData.reglasPropiedad
          : [],

        estado: propertyData.estado
          ? propertyData.estado.charAt(0).toUpperCase() +
            propertyData.estado.slice(1).toLowerCase()
          : "Disponible",
      ...
      if (propertyData.availability && propertyData.availability.length > 0) {
        propertyToSave.availability = propertyData.availability.map(
          (avail) => ({
            startDate: new Date(avail.startDate),
            endDate: new Date(avail.endDate),
            status: "disponible",
      ...
      const propertyId = isEditing
        ? propertyToEdit.id || propertyToEdit._id
        : null;
      ...
      if (response.status && response.value) {
        ...
        if (imageFiles && imageFiles.length > 0) {
          const uploadResponse = await propertyService.uploadImages(
            savedPropertyId,
            imageFiles,
          );
          ...
          if (!uploadResponse.status) {
            throw new Error(
              "Propiedad guardada pero error al subir las imágenes",
            );
          }
        ...
        if (isEditing) {
            ...
            prev.map((p) => {
              if (p.id === propertyId || p._id === propertyId) {
                return {
                  ...
                  title:
                    finalProperty.title ||
                    finalProperty.titulo ||
                    p.title ||
                    "Sin título",
                  description:
                    finalProperty.description ||
                    finalProperty.descripcion ||
                    p.description ||
                    "",
                  price:
                    finalProperty.price || finalProperty.precio || p.price || 0,
                  images:
                    finalProperty.images ||
                    finalProperty.imagenes ||
                    p.images ||
                    [],
                  services:
                    finalProperty.servicios ||
                    finalProperty.services ||
                    p.services ||
                    [],
                  rules:
                    finalProperty.reglasPropiedad ||
                    finalProperty.rules ||
                    p.rules ||
                    [],
                };
          ...
          const newProperty = {
            ...
            description:
              finalProperty.description || finalProperty.descripcion || "",
  ...
  const handleDeleteProperty = async (propertyId) => {
    if (
      !window.confirm("¿Está seguro de eliminar esta propiedad temporaria?")
    ) {
      return;
    }
      ...
      <AdminLayout>
        <div className="text-center py-20">
          ...
          <p className="text-gray-600">
            Cargando propiedades de alquiler temporario...
          </p>
        </div>
      </AdminLayout>
    );
      ...
      {isFormOpen && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">
            {propertyToEdit
              ? "Editar Propiedad Temporaria"
              : "Nueva Propiedad Temporaria"}
          </h2>
          {propertyToEdit ? (
            <EditPropertyForm
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-200">
              ...
              <div className="text-center p-3 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">
                  {
                    currentFilteredProperties.filter(
              ...
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">
                  $
                  {Math.round(
                    currentFilteredProperties.reduce(
                      (sum, p) => sum + p.price,
                      0,
              ...
              <div>
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex items-center text-gray-700">
                    ...
                    <span>
                      {selectedProperty.locality}, {selectedProperty.address}
                    </span>
                  </div>
                  <div className="flex items-center text-gray-700">
              ...
              <div>
                <ReservationCalendar
                  ...
                  onReserve={(reservation) =>
                    handleReserve(selectedProperty.id, reservation)
                  }
                  users={users}
