...
const PropertyForm = ({
  ...
  const handleSave = () => {
    if (!formData.title || !formData.address || !formData.price) {
      alert(
        "Por favor complete los campos requeridos: Título, Dirección y Precio.",
      );
      return;
    }
    ...
    if (editing && !propertyId) {
      alert(
        "Error: No se encontró el ID de la propiedad. Recarga la página e intenta nuevamente.",
      );
      return;
    }
    ...
    if (typeof onSave === "function") {
      onSave(propertyDataToSave, newImageFiles, removedIds);
    }
  };
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <FaSave />
              <span>
                {editing ? "Actualizar Propiedad" : "Guardar Propiedad"}
              </span>
            </>
          )}
        </button>
