...
const PropertyManagement = () => {
  ...
  const [formProperty, setFormProperty] = useState(() =>
    createEmptyProperty(selectedOperation),
  );
  ...
  const filterProperties = (properties, filters, searchTerm) => {
    return properties.filter((property) => {
      ...
      const maxPrice =
        Number(filters.priceRange.max) === Infinity
          ? Infinity
      ...
      const matchesBedrooms =
        !filters.bedrooms ||
        (property.bedrooms || 0) >= parseInt(filters.bedrooms || "0", 10);
      const matchesBathrooms =
        !filters.bathrooms ||
        (property.bathrooms || 0) >= parseInt(filters.bathrooms || "0", 10);
      const matchesPriceRange =
        propertyPrice >= minPrice && propertyPrice <= maxPrice;
      const matchesStatus =
        !filters.status || property.status === filters.status;
      const matchesPets =
        filters.allowsPets === null ||
        property.allowsPets === (filters.allowsPets === "true");
      const matchesSearch =
        !searchTerm ||
        (property.title &&
  ...
  const filteredAndSortedProperties = sortProperties(
    filterProperties(properties, filters, searchTerm),
    sortBy,
  );
  ...
  const handleSaveProperty = async (
    ...
    try {
      ...
      const backendData = {
        ...
        transaccionTipo:
          propertyData.operationType === "venta" ? "Venta" : "Alquiler",
      ...
      if (isEdit && editingProperty.id) {
        ...
        response = await propertyService.update(
          editingProperty.id,
          backendData,
        );
        if (response.status) {
      ...
      if (targetPropertyId && imageFiles && imageFiles.length > 0) {
        ...
        try {
          const uploadResp = await propertyService.uploadImages(
            targetPropertyId,
            imageFiles,
          );
                ...
                prevProperties.map((prop) =>
                  prop.id === targetPropertyId
                    ? {
                        ...prop,
                        images:
                          uploadResp.value.imagenes ||
                          uploadResp.value.images ||
                          [],
                      }
                    : prop,
                ),
          ...
          showNotification(
            "Propiedad guardada pero hubo error al subir imágenes",
            "error",
          );
        }
      }
      ...
      if (createdOrUpdatedId) {
        try {
          ...
          console.warn(
            "No se pudo refrescar la propiedad desde el backend:",
            fetchError,
          );
        }
      }
      ...
      if (editingProperty && editingProperty.id) {
        ...
        setProperties((prevProperties) => {
          const withoutPlaceholder = prevProperties.filter(
            (prop) => prop.id !== propertyToStore.id,
          );
      ...
      if (deletionErrors.length > 0) {
        showNotification(
          `Se guardó la propiedad, pero no se pudieron eliminar ${deletionErrors.length} imágenes.`,
          "error",
        );
      }
    } catch (error) {
      ...
      showNotification(
        `Error al ${editingProperty ? "actualizar" : "crear"} la propiedad: ${error.message}`,
        "error",
      );
    } finally {
    ...
    <AdminLayout>
      {view === "form" && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <PropertyForm
            key={
              editingProperty ? `edit-${editingProperty.id || "new"}` : "new"
            }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-200">
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">
                  {
                    properties.filter(
                      (p) => p.operationType === selectedOperation,
                    ).length
                  }
                </div>
                ...
                <div className="text-2xl font-bold text-green-600">
                  {
                    properties.filter(
                      (p) =>
                        p.operationType === selectedOperation &&
                        p.status === "disponible",
                    ).length
                  }
                </div>
