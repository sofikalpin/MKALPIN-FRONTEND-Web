...
export const propertyService = {
  ...
  search: async (filters) => {
    ...
    if (filters.transaccionTipo)
      queryParams.append("transaccionTipo", filters.transaccionTipo);
    if (filters.tipoPropiedad)
      queryParams.append("tipoPropiedad", filters.tipoPropiedad);
    ...
    if (filters.habitacionesMin)
      queryParams.append("habitacionesMin", filters.habitacionesMin);
    if (filters.banosMin) queryParams.append("banosMin", filters.banosMin);
    if (filters.superficieMin)
      queryParams.append("superficieMin", filters.superficieMin);
    if (filters.superficieMax)
      queryParams.append("superficieMax", filters.superficieMax);
    if (filters.estado) queryParams.append("estado", filters.estado);
    if (filters.esAlquilerTemporario !== undefined)
      queryParams.append("esAlquilerTemporario", filters.esAlquilerTemporario);

    const response = await api.get(
      `/Propiedad/Buscar?${queryParams.toString()}`,
    );
    if (response.status && response.value) {
      response.value = response.value.map(mapPropertyData);
    }
  ...
  deleteImage: async (propertyId, imageId) => {
    if (!propertyId || !imageId) {
      throw new Error(
        "Property id and image id are required to delete an image",
      );
    }
    return await api.delete(
      `/Propiedad/EliminarImagen/${propertyId}/${imageId}`,
    );
  },
  ...
  createWithImages: async (propertyData, imageFiles = []) => {
    ...
    if (createResp.status && createResp.value && imageFiles.length > 0) {
      ...
      if (propId) {
        await api.post(
          `/Propiedad/SubirImagenes/${propId}`,
          (() => {
...
export const reservationService = {
  ...
  update: (id, reservation) =>
    api.put(`/Reserva/Actualizar/${id}`, reservation),
  delete: (id) => api.delete(`/Reserva/Eliminar/${id}`),
  getByProperty: (propertyId) =>
    api.get(`/Reserva/Obtener/Propiedad/${propertyId}`),
};
    ...
    try {
      ...
      return {
        ...
        value: {
          ...
          propiedadesDisponibles: properties.filter(
            (p) => p.estado === "Disponible" || p.disponible,
          ).length,
          propiedadesOcupadas: properties.filter(
            (p) => p.estado === "Ocupado" || !p.disponible,
          ).length,
          propiedadesPorVenta: properties.filter(
            (p) => p.transaccionTipo === "Venta",
          ).length,
          propiedadesPorAlquiler: properties.filter(
            (p) => p.transaccionTipo === "Alquiler",
          ).length,
          propiedadesTemporario: properties.filter(
            (p) => p.esAlquilerTemporario,
          ).length,
